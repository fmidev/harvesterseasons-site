...
var dyGraphBOptions2 = {
    ...
    // dateWindow: [new Date('2021-11-02'), new Date('2021-11-22')],
...
if (!urlParams.has('tradecont')) {
    tradecont = "131468895";
} else {
    // tradecont = urlParams.getAll('tradecont')[0].replace(";", "\n");
    tradecont = urlParams.getAll('tradecont')[0].split(",");
}

if (urlParams.has('dates')) {
    urlDates = urlParams.getAll('dates')[0].split(",");
    // dyGraphBOptions2.dateWindow = [new Date('2021-11-02'), new Date('2021-11-22')];
    dyGraphBOptions2.dateWindow = [new Date(urlDates[0]), new Date(urlDates[1])];
}

// console.debug(tradecont)
...
function drawtimeseries() {
    ...
    var dataLoad = $.getJSON("https://harvesterseasons.com/custom/labels.json",
        function (data) {
            ...
            // console.debug(data.features[0].properties["Trade Cont"])
            // console.debug(data.features[0].properties["Trade Cont"])
            // console.debug(data.features.findIndex(n => n.properties["Trade Cont"] === tradecont))

            // for (var n = 0; n < data.features.length; n++) {
            // for (var n = 0; n < 10; n++) {
            // for (var n = 0; n < 1; n++) {
            for (var n = 0; n < tradecont.length; n++) {

                var dataIndex = data.features.findIndex(x => x.properties["Trade Cont"] === tradecont[n]);

                // console.debug(data.features[n].properties["Trade Cont"])

                var lat = data.features[dataIndex].geometry.coordinates[1];
                var lon = data.features[dataIndex].geometry.coordinates[0];
                var sitename = data.features[dataIndex].properties["Trade Cont"];
...
function drawtimeseries_ecbsf(lat, lon, titleID, labelsID, graphID, graphblockID, gB, sitename, plotID, histVec) {
    ...
    var graphCssText2 = "display: inline-block; height: 100px; width: 90%; padding-left: 10px; margin-top: 0px; margin-bottom: 0px; text-align: center;";
