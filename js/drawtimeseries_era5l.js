function drawtimeseries() {
    ...
    var dataUrlSW = "https://desm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,SWI2-0TO1:SWIC:5059,SWI2-0TO1:SWI:5059,SWI2-0TO1:EDTE:5068,SWVL2-M3M3:SMARTMET:5015" + SWensemble + "&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&format=json&precision=full&source=grid&timeformat=sql&tz=utc";
    ...
    $.getJSON(dataUrlSW, function (dataSW) {
        ...
        if (smartmetIdx == -1) {
            ...
            $.getJSON(dataUrlSH, function (dataSH) {
                ...
                // let summer1series = [];
                // summer1series = harvidx(0.69, dataSWscaled, SWensemblelist, perturbations, "SWI-0TO1:SWI:5059")
                ...
                graphLoad = $.getJSON("https://desm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,"
                 ...
                 + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&format=json&source=grid&timeformat=xml&tz=utc",
                    function (data) {
                        ...
                        for (k = 0; k < dataSW.length; k++) {
                           ...
                           dataSW2[k][perturbations + 2] = dataSW[k]["SWI2-0TO1:SWIC:5059"];
...
function drawOutsideFinland() {
    ...
    graphLoad = $.getJSON("https://desm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param1 + "," + param2 + "," + param3 + "," + param4 + "," + param5 + "," + param7 + "&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&format=json&source=grid&timeformat=xml&tz=utc",
        function (data) {
            ...
            graphLoad3 = $.get("https://desm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime" + SWensemble + ",SWI2-0TO1:SWIC:5059,SWI2-0TO1:EDTE:5068,SWI2-0TO1:SWI:5059&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc",
                function (data) {
                    if (data.length > 0) {
                        gsw = new Dygraph(
                            document.getElementById("graphsw"),
