...
param4="ensover{0.9;0.4;HSNOW-M:ECB2SF::1:0:3:1-50;HSNOW-M:ECB2SF::1:0:1:0}";
...
var snowthicknessLayerOptions = {
    crs: L.CRS.EPSG4326,
    version: '1.3.0',
    layers: 'harvester:ecb2sf:HSNOW-M',
    format: 'image/png',
    transparent: 'true',
    styles: 'default',
    //source: 'grid',
...
for (i = 1; i <= perturbations; i = i + 1) {
    label[i+1] = 'SH-' + i ;
    labelstxt[label[i]]= { fillGraph: false };
    SHensemble += ",HSNOW-M:ECB2SF::1:0:3:" + i ;
}
...
function drawtimeseries() {
    graphLoad = $.getJSON("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param1 + "," + param2 + "," + param3 + "," + param4 + "&starttime=data&endtime=data&timestep=data&format=json&precision=full&source=grid&timeformat=xml",
        function (data) {
            ...
            graphLoad2 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,HSNOW-M:ECB2SF::1:0:1:0" + SHensemble + "&starttime=data&endtime=data&timestep=data&timeformat=sql&precision=full&separator=,&source=grid",
                function (data) {
                    if (data.length > 0) {
                        gsh = new Dygraph(
