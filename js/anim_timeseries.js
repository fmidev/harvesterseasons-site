...
const param8 = "ensover{0.4;0.9;HSNOW-M:SMARTOBS}";
const param9 = "HARVIDX{0.2;SWVL2-M3M3:SMARTMET}";
const param10 = "HARVIDX{-0.7;TG-K:SMARTOBS}";
...
for (p = 1; p <= perturbations; p++) {
    SHensemblelist[p] = "SD-M:ECB2SF::1:0:3:" + p ;
    SHensemble2 += ",DIFF{SD-M:ECB2SF::1:0:3:" + p + ";HSNOW-M:SMARTOBS:13:4}";
    SHensemble2list[p] = "DIFF{SD-M:ECB2SF::1:0:3:" + p + ";HSNOW-M:SMARTOBS:13:4}";
}
...
function drawtimeseries() {
    ...
    for (var n = 0; n < lines.length; n++) {
        if (lines[n].length > 0) {
            var lat = parseFloat(lines[n].split(",")[0]);
            var lon = parseFloat(lines[n].split(",")[1]);
            ...
            drawtimeseries_ecbsf(lat, lon, "titleB" + graphIdx, "labelsB" + graphIdx, "graphB" + graphIdx, graphIdx);
...
function drawtimeseries_ecbsf(lat, lon, titleID, labelsID, graphID, gB) {
    ...
    document.getElementById(graphID).style.cssText = "display: inline-block; height: 100px; width: 90%; padding-left: 10px; margin-top: 0px; margin-bottom: 0px; text-align: center;"
    ...
    document.getElementById(graphID).style.lineHeight = "120px";
    ...
    if (!inFinland(lat, lon)) {
        ...
        // graphLoad = $.getJSON("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param1 + "," + param5 + "," + param6 + "," + param7 + "&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&format=json&source=grid&timeformat=xml&tz=utc",
        function (data) {
                var graphdata = [];
                ...
                if (graphdata.length > 0) {
                    ...
                    // // console.debug(gB, graphList.length, graphIdx)
                    // // if (graphList.length > 1) {
                    // if (graphIdx > 1) {
                    //     if (gB < graphIdx - 1) {
                    //         console.debug(gB, graphList.length, graphIdx)
                    //         graphList[gB].updateOptions(dyGraphBOptions2);
                    //     }
                    //     var sync = Dygraph.synchronize(graphList);
                    // }
        ...
        $.getJSON(dataUrl2, function (data2) {
            ...
            for (p = 1; p <= perturbations; p++) {
                SHensemble3 += ",DIFF{SD-M:ECB2SF::1:0:3:" + p + ";" + data2[0][SHensemble2list[p]] + "}";
                SHensemble3list[p] = "DIFF{SD-M:ECB2SF::1:0:3:" + p + ";" + data2[0][SHensemble2list[p]] + "}";
                SHensemble3ensover += ";DIFF{SD-M:ECB2SF::1:0:3:" + p + ";" + data2[0][SHensemble2list[p]] + "}";
            }
            ...
            // graphLoad = $.getJSON("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param1 + "," + param3 + "," + param4ensemble + "," + param5 + "," + param6 + "," + param7 + "," + param8 + "&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&format=json&source=grid&timeformat=xml&tz=utc",
                function (data) {
                    var graphdata = [];
                    for (var i = 0, k = 0; i < data.length; i++) {
                        var summer1, summer2, winter1, winter2;
