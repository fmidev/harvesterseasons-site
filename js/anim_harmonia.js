...
// https://desm.harvesterseasons.com/wms?&SERVICE=WMS&REQUEST=GetMap&VERSION=1.3.0&LAYERS=harvester:s3sy:NDVI&STYLES=&FORMAT=image/png&TRANSPARENT=true&HEIGHT=800&WIDTH=800&20220111T000000&CRS=EPSG:4326&BBOX=34,-10,71,50
...
snowLegend.onAdd = function (map) {
    var src = 'https://desm.harvesterseasons.com/wms?REQUEST=GetLegendGraphic&VERSION=1.3.0&LAYER=harvester:ecbsf:FF-MS&sld_version=1.1.0&style=&FORMAT=image/png&WIDTH=60&HEIGHT=185';
    var div = L.DomUtil.create('div', 'info legend');
    if (screen.width < 425) {
...
soilwetLegend.onAdd = function (map) {
    ...
    var src = 'https://desm.harvesterseasons.com/wms?REQUEST=GetLegendGraphic&VERSION=1.3.0&LAYER=harvester:ecbsf:RR-M&sld_version=1.1.0&style=&FORMAT=image/png&WIDTH=65&HEIGHT=345';
    var div = L.DomUtil.create('div', 'info legend');
    if (screen.width < 425) {
        // div.style.width = '65px';
        // div.style.height = '325px';
...
soilwetLegend2.onAdd = function (map) {
    // var src = 'https://desm.harvesterseasons.com/wms?REQUEST=GetLegendGraphic&VERSION=1.3.0&LAYER=harvester:ecbsf:SOILWET-M3M3&sld_version=1.1.0&style=&FORMAT=image/png&WIDTH=65&HEIGHT=345';
    // var src = 'https://desm.harvesterseasons.com/wms?REQUEST=GetLegendGraphic&VERSION=1.3.0&LAYER=harvester:ecxb2sf:RR-M&sld_version=1.1.0&style=&FORMAT=image/png&WIDTH=65&HEIGHT=345';
    var src = 'https://desm.harvesterseasons.com/wms?REQUEST=GetLegendGraphic&VERSION=1.3.0&LAYER=harvester:ecbsf:RR-M&sld_version=1.1.0&style=&FORMAT=image/png&WIDTH=65&HEIGHT=345';
    var div = L.DomUtil.create('div', 'info legend');
    if (screen.width < 425) {
        // div.style.width = '65px';
...
/* fireLegend.onAdd = function (map) {
    //var src = 'https://desm.harvesterseasons.com/wms?service=WMS&request=GetLegendGraphic&version=1.3.0&sld_version=1.1.0&style=default&format=image%2Fpng&layer=fmi%3Akosteusmalli%3A10km%3Aforestfireindexlegend&WIDTH=65&HEIGHT=110';
...
var overlayMaps = {
    "Acc. Precipitation": soilwetnessTimeLayer,
    "Acc. Precipitation ECXB2SF": soilwetnessTimeLayer2,
    "2 m Temperature": temperatureTimeLayer.addTo(map),
    // "Soil Temperature": temperatureTimeLayer,
    "Wind Speed": snowthicknessTimeLayer,
...
map.on('overlayremove', function (e) {
    ...
    switch (e.name) {
        ...
        case "Acc. Precipitation": {
            map.removeControl(soilwetLegend);
            break;
        }
        case "Acc. Precipitation ECXB2SF": {
            map.removeControl(soilwetLegend2);
            break;
        }
        case "2 m Temperature": {
...
map.on('overlayadd', function (e) {
    ...
    switch (e.name) {
        case "Wind Speed": {
            ...
            else if (map.hasLayer(soilwetnessTimeLayer2)) { 
        ...
        case "Acc. Precipitation ECXB2SF": {
            ...
            if (!soilwetnessTimeLayer2._currentLayer._map) {
                soilwetnessTimeLayer2.setParams({});
            }
            soilwetLegend2.addTo(this);
            // lcontrol._overlaysList.children[5].control.disabled = true;
            // lcontrol._overlaysList.children[5].style.color = "rgb(190, 190, 190)";
            break;
        }
...
// Accumulated Precipitation RR-M

var SWensemble = "";
var label = ["date", "AP-0"];
var labelstxt = {'AP-0': { fillGraph: false }};
...
/*     label[i+2] = 'SW-' + i ;
    ...
    SWensemble += ",RR-M:ECBSF:5022:1:0:3:" + i ;
}
...
/*     label[i+2] = 'SW-' + i ;
    labelstxt[label[i+2]]= { fillGraph: false }; */
    // SWensemble += ",SOILWET-M3M3:ECBSF::9:7:3:" + i ;
    SWensemble2 += ",RR-M:ECXB2SF:5021:1:0:3:" + i ;
}
...
var dyGraphSWOptions = {
    legend: "always",
    ylabel: "Accumulated Precipitation (m)",
...
var dyGraphRROptions = {
    legend: "always",
    ylabel: "Accumulated Precipitation ECXB2SF (m)",
    // ylabel: "Soil Wetness (m\u00B3/m\u00B3)",
    // ylabel: "Snow Density (kg/m\u00B3)",
    labels: label,
    series: labelstxt,
...
var label = ["date", "T2-0"];
var labelstxt = {'T2-0': { fillGraph: false }};
...
for (i = 1; i <= perturbations; i = i + 1) {
    ...
    TGKensemble = TGKensemble + ",K2C{T2-K:ECBSF:5022:1:0:3:" + i + "}";
}
...
// var SHensemble = "";
var SHensemble = "FF-MS:ECBSF:5022:1:0:1:0";
var label = ["date", "WS-0"];
var labelstxt = {'WS-0': { fillGraph: false }};
for (i = 1; i <= perturbations; i = i + 1) {
    label[i+1] = 'WS-' + i ;
    labelstxt[label[i+1]]= { fillGraph: false };
    SHensemble += ",FF-MS:ECBSF:5022:1:0:3:" + i ;
}
...
function altcolors() {
    ...
    location.href = "https://harvesterseasons.com/index_ndvi_altcolors.html#" + urlpos.center.lat + "," + urlpos.center.lng + "," + urlpos.zoom + "z";
}
