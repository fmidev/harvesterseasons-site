...
function plotSW() {
    ...
    const paramSW = "SWVL2-M3M3:SMARTMET:5015";

    var dataUrlSW = "https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime," + paramSW + "&starttime=" + dateString_smartobs + "T000000Z&endtime=" + dateString_smartmet + "&timestep=1440&format=json&precision=full&tz=utc&timeformat=xml";

    // Soil wetness scaled with SWI
    // var dataUrlSW = "https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + paramSW + "&starttime=" + dateString_swi + "T000000Z&timesteps=1&format=json&precision=full";
    $.getJSON(dataUrlSW, function (dataSW) {
        // console.debug(dataUrlSWI)
        // console.debug(dataSWI[0])
        // console.debug(dataSWI[0][paramSWI])

        let swiValue = dataSWI[0][paramSWI] / 100;

        if (swiValue > 0) {

            // var soilwet1 = "DIFF{SOILWET-M3M3:ECBSF:::7:1:0;" + swiValue + "}"

            // var SWensemblelist = ["SOILWET-M3M3:ECBSF:::7:1:0"];
            var SWensemble2 = "DIFF{SOILWET-M3M3:ECBSF:::7:1:0;" + swiValue + "}";
            var SWensemble2list = ["DIFF{SOILWET-M3M3:ECBSF:::7:1:0;" + swiValue + "}"];
            for (i = 1; i <= perturbations; i = i + 1) {
                // SWensemblelist[i] = "SOILWET-M3M3:ECBSF:::7:3:" + i ;
                SWensemble2 += ",DIFF{SOILWET-M3M3:ECBSF:::7:3:" + i + ";" + swiValue + "}";
                SWensemble2list[i] = "DIFF{SOILWET-M3M3:ECBSF:::7:3:" + i + ";" + swiValue + "}";
            }

            var dataUrlDiff = "https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + SWensemble2 + "&starttime=" + dateString_swi + "T000000Z&timesteps=1&format=json&precision=full";
            $.getJSON(dataUrlDiff, function (dataDiff) {

                // console.debug(dataDiff[0][soilwet1])
                // console.debug(dataDiff[0])

                // var swiDiff = dataDiff[0][soilwet1];
                // var soilwet2 = "DIFF{SOILWET-M3M3:ECBSF:::7:1:0;" + swiDiff + "}"

                // Scale seasonal orecast using SWI
                var SWensemble3 = "DIFF{SOILWET-M3M3:ECBSF:::7:1:0;" + dataDiff[0][SWensemble2list[0]] + "}";
                // var SWensemble3ensover = "DIFF{SOILWET-M3M3:ECBSF:::7:1:0;" + dataDiff[0][SWensemble2list[0]] + "}";
                var SWensemble3list = ["DIFF{SOILWET-M3M3:ECBSF:::7:1:0;" + dataDiff[0][SWensemble2list[0]] + "}"];
                for (i = 1; i <= perturbations; i = i + 1) {
                    SWensemble3 += ",DIFF{SOILWET-M3M3:ECBSF:::7:3:" + i + ";" + dataDiff[0][SWensemble2list[i]] + "}";
                    SWensemble3list[i] = "DIFF{SOILWET-M3M3:ECBSF:::7:3:" + i + ";" + dataDiff[0][SWensemble2list[i]] + "}";
                    // SWensemble3ensover += ";DIFF{SOILWET-M3M3:ECBSF:::7:3:" + i + ";" + dataDiff[0][SWensemble2list[i]] + "}";
                }

                var dataUrlSWIserie = "https://smdev.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime," + paramSWI + "&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc&format=json";
                $.getJSON(dataUrlSWIserie, function (dataSWIserie) {
                    ...
                    // graphLoad3 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime," + SWensemble3 + ",SWVL2-M3M3:SMARTMET:5015&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc&format=json",
                    // graphLoad3 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime," + SWensemble3 + ",SWVL2-M3M3:SMARTMET:5015&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc",
                    graphLoad3 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime," + SWensemble3 + "&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc&format=json",
                        function (data) {
                            if (data.length > 0) {
                                ...
                                for (k = 0; k < data.length; k++) {
                                    // console.debug(data[k]["utctime"])
                                    dataSW[k] = [];
                                    dataSW[k][0] = new Date(data[k]["utctime"]);
                                    // if (k == 0) {
                                    //     // console.debug(dataSW[k])
                                    //     console.debug(data[k]["utctime"])
                                    //     console.debug(startDate_swi - 24 * 60 * 60000)

                                    // }
                                    for (i = 1; i <= perturbations + 1; i = i + 1) {
                                        if (dataSW[k][0] < startDate_swi - 24 * 60 * 60000) {
                                            // Remove seasonal forecast before startDate_smartobs-1day
                                            dataSW[k][i] = "nan";
                                            // } else if (data[k][SHensemblelist[i]] == 0 || data[k][SHensemble3list[i]] < 0) {
                                            //     // Set SD to 0 if non-scaled SD is 0 or scaled < 0
                                            ...
                                            dataSW[k][i] = data[k][SWensemble3list[i]];
                                        }
                                    }
                                    // dataSW[k][perturbations + 2] = data[k]["SWVL2-M3M3:SMARTMET:5015"];
                                    // if (data.length == dataSWIserie.length) {
                                    //     dataSW[k][perturbations + 2] = dataSWIserie[k][paramSWI];
                                    // }
                                    if (data.length == dataSWIserie.length && dataSWIserie[k][paramSWI] > 0) {
                                        dataSW[k][perturbations + 2] = dataSWIserie[k][paramSWI] / 100;
                                    } else {
                                        dataSW[k][perturbations + 2] = "nan";
                                    }
                                }

                                // console.debug(dataSWIserie)
                                // console.debug(data.length)
                                // console.debug(dataSWIserie.length)
                                // console.debug(dataSWI)
            ...
            // graphLoad3 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,SOILWET1-M:ECBSF::9:7:1:0" + SWensemble + ",SWVL2-M3M3:SMARTMET:5015&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc",
            // graphLoad3 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,SOILWET-M3M3:ECBSF:::7:1:0" + SWensemble + ",SWVL2-M3M3:SMARTMET:5015&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc",
            graphLoad3 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,SOILWET-M3M3:ECBSF:::7:1:0" + SWensemble + "&starttime=" + dateString_timeseries + "&endtime=" + dateString_ecbsf + "&timestep=1440&timeformat=sql&precision=full&separator=,&source=grid&tz=utc",
                function (data) {
                    if (data.length > 0) {
                        gsw = new Dygraph(
                            document.getElementById("graphsw"),
