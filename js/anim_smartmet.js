...
//param1="utctime";
//param2="HARVIDX{0.4;SOILWET1-M:ECBSF:5022::7:3:1-50;SOILWET1-M:ECBSF:5022::7:1:0}";
//param3="HARVIDX{273;TSOIL-K:ECBSF:5022::7:3:1-50;TSOIL-K:ECBSF:5022::7:1:0}";
//param4="ensover{0.4;0.9;SD-M:ECBSF:5022:1:0:3:1-50;SD-M:ECBSF:5022:1:0:1:0}";
...
function plotgeotiff() {
    ...
    if (harvDynamic.checked) {
        ...
        //dataUrl2 = "https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param2 + "," + param3 + "," + param4 + "&starttime=" + dataYear + dataMonth + dataDay + "T000000Z&timesteps=1&format=json";

        // No snow data
        dataUrl2 = "https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param2 + "," + param3 + "&starttime=" + dataYear + dataMonth + dataDay + "T000000Z&timesteps=1&format=json";
...
var temperatureLayerOptions = {
    ...
    //layers: 'harvester:fmi:fmi_soil_temperature',
    layers: 'harvester:smartmet:TG-K',
    format: 'image/png',
    transparent: 'true',
    styles: 'default',
    //source: 'grid',
...
var soilwetnessLayerOptions = {
    ...
    //layers: 'harvester:fmi:fmi_soil_water',
...
var snowthicknessLayerOptions = {
    ...
    layers: 'harvester:smartmet:SD-M',
    format: 'image/png',
    transparent: 'true',
    styles: 'default',
    //source: 'grid',
...
var overlayMaps = {
    "Soil Wetness": soilwetnessTimeLayer.addTo(map),
...
function drawtimeseries() {
    //graphLoad = $.getJSON("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param1 + "," + param2 + "," + param3 + "," + param4 + "&starttime=data&endtime=data&timestep=data&format=json&source=grid&timeformat=xml",

    // No snow data
    graphLoad = $.getJSON("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=" + param1 + "," + param2 + "," + param3 + "&starttime=data&endtime=data&timestep=data&format=json&source=grid&timeformat=xml",
        function (data) {
            var graphdata = [];
            ...
            for (i = 0, j = 0; i < data.length; i++) {
                //console.debug(data[i][param1][11])
                //console.debug(data[i][param1][11]==0)
                //graphdata[i] = [new Date(data[i][param1]), data[i][param2], data[i][param3]];

                // filter out 12:00 observations
                if (data[i][param1][11] == 0) {
                    graphdata[j] = [new Date(data[i][param1]), data[i][param2], data[i][param3]];
                    //console.debug(graphdata[j])
                    j++;
                }
            }
            ...
            graphLoad3 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,SWVL2-M3M3:SMARTMET:5015&starttime=data&endtime=data&timestep=data&timeformat=sql&precision=full&separator=,&source=grid",
                function (data) {
                    if (data.length > 0) {
                        gsw = new Dygraph(
                        ...
                        if (typeof gsw !== 'undefined' && typeof gst !== 'undefined' && typeof gsh !== 'undefined') {
                            ...
                            gst.updateOptions({dateWindow: gst.xAxisExtremes()})                    
                        }
                    } else {
            ...
            graphLoad4 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,TG-K:SMARTMET&starttime=data&endtime=data&timestep=data&timeformat=sql&separator=,&precision=full&source=grid",
                function (data) {
                    if (data.length > 0) {
                        gst = new Dygraph(
            ...
            graphLoad2 = $.get("https://sm.harvesterseasons.com/timeseries?latlon=" + latlonPoint + "&param=utctime,SD-M:SMARTMET:5027&starttime=data&endtime=data&timestep=data&timeformat=sql&precision=full&separator=,&source=grid",
                function (data) {
                    if (data.length > 0) {
                        gsh = new Dygraph(
